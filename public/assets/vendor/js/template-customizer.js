...
var TemplateCustomizer = /*#__PURE__*/function () {
  ...
  return _createClass(TemplateCustomizer, [{
    ...
    value: function setStyle(style) {
      ...
      var isAdmin = !layoutName.includes('front');
      this._setSetting('Style', style);
      var modeCookieName = isAdmin ? 'admin-mode' : 'front-mode';
      var colorPrefCookieName = isAdmin ? 'admin-colorPref' : 'front-colorPref';
    ...
    value: function _loadSettings() {
      ...
      this.settings.rtl = this._checkCookie('direction') ? this._getCookie('direction') : rtl !== '' ? rtl === 'true' : this.settings.defaultTextDir;
    ...
    value: function _setup() {
      ...
      var resetBtnCb = function resetBtnCb() {
        var layoutName = _this2._getLayoutName();
        if (layoutName.includes('front')) {
          _this2._deleteCookie('front-mode');
          _this2._deleteCookie('front-colorPref');
        } else {
          _this2._deleteCookie('admin-mode');
          _this2._deleteCookie('admin-colorPref');
        }
        _this2.clearLocalStorage();
        window.location.reload();
        _this2._deleteCookie('colorPref');
        _this2._deleteCookie('theme');
        _this2._deleteCookie('direction');
      };
      ...
      if (!this._hasControls('rtl headerType contentLayout layoutCollapsed layoutNavbarOptions', true)) {
        ...
        if (!this._hasControls('rtl') || !rtlSupport) {
          ...
          directionOpt.querySelector("input[value=\"".concat(this.settings.rtl === 'true' ? 'rtl' : 'ltr', "\"]")).setAttribute('checked', 'checked');
    ...
    value: function _initDirection() {
      if (this._hasControls('rtl')) {
        document.documentElement.setAttribute('dir', this._checkCookie('direction') ? this._getCookie('direction') === 'true' ? 'rtl' : 'ltr' : this.settings.rtl ? 'rtl' : 'ltr');
      }
    }
