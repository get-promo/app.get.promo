    ...
    try:
        ...
        with open(CITIES_PATH, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                parts = line.split(',')
                if len(parts) >= 2:
                    location = parts[0].strip()  # "Warszawa Mokotów"
                    population = parts[1].strip()
                    locations.append({'location': location, 'population': population})
        
        logger.info(f"Wczytano {len(locations)} lokalizacji (miasto + dzielnica)")
        return locations
    except Exception as e:
        logger.error(f"Błąd wczytywania lokalizacji: {e}")
        raise
...
def process_place(place, search_phrase, location, population):
...
def process_combination(phrase, location_data, progress):
    """
    Przetwarza jedną kombinację fraza+lokalizacja.
    Pobiera dane z Serper API (wszystkie strony dopóki resultsCount == 10) i zapisuje do bazy.
    """
    location = location_data['location']  # "Warszawa Mokotów"
    population = location_data['population']
    
    combination_key = f"{phrase}|{location}"
    ...
    logger.info(f"Przetwarzanie: {phrase} - {location}")
    
    connection = None
    local_stats = {'added': 0, 'updated': 0, 'errors': 0, 'pages': 0}
    
    try:
        connection = get_db_connection()
        cursor = connection.cursor()
        
        search_query = f"{phrase} {location}"
        
        # Pobieranie stron dopóki resultsCount == 10
        page = 1
        while True:
            ...
            logger.info(f"Znaleziono {results_count} miejsc dla '{search_query}' strona {page}")
            
            for place in places:
                place_data = process_place(place, phrase, location, population)
            ...
            # WARUNEK KONTYNUACJI: Jeśli wyników < 10, kończymy
            if results_count < 10:
                logger.info(f"Zakończono dla '{search_query}' - ostatnia strona miała {results_count} wyników")
                break
        ...
        progress[combination_key] = {
            ...
            'pages': local_stats['pages']
        }
        save_progress(progress)
        
        logger.info(f"Zakończono: {combination_key} | Strony: {local_stats['pages']}, Dodano: {local_stats['added']}, Zaktualizowano: {local_stats['updated']}, Błędy: {local_stats['errors']}")
...
def main():
    ...
    logger.info(f"Liczba lokalizacji (miasto+dzielnica): {len(locations)}")
    ...
    for phrase in phrases:
        for location_data in locations:
            combination_key = f"{phrase}|{location_data['location']}"
            if not progress.get(combination_key, {}).get('completed'):
                tasks.append((phrase, location_data))
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        ...
        for future in as_completed(futures):
            phrase, location_data = futures[future]
            try:
                future.result()
            except Exception as e:
                logger.error(f"Nieobsłużony błąd dla {phrase} - {location_data['location']}: {e}")
    ...
    logger.info(f"Zapytań do Serper API: {stats['total_queries']}")
    logger.info("=" * 80)
